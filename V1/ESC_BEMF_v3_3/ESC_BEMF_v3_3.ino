...
void setup() {
  ...
  PCMSK2 |= (1 << PCINT18);  //Set pin D8 trigger an interrupt on state change. 
  sei();
  delay(200);
...
//D9 PWM and D4 HIGH.  
void AH_BL(){
  PORTD = B01000000;      //Set D4 to HIGH and the rest to LOW
  TCCR2A =  0;            //OC2A - D11 normal port. 
  TCCR1A =  0x21;         //OC1B - D9 compare match noninverting mode, downcounting ,PWM 8-bit
}
//D9 PWM and D3 HIGH
void AH_CL(){
  PORTD = B00100000;      //Set D3 to HIGH and the rest to LOW
  TCCR2A =  0;            //OC2A - D11 normal port. 
  TCCR1A =  0x21;         //OC1B - D9 compare match noninverting mode, downcounting ,PWM 8-bit  
}
//D10 PWM and D3 HIGH
void BH_CL(){
  PORTD = B00100000;      //Set D3 to HIGH and the rest to LOW
  TCCR2A =  0;            //OC2A - D11 normal port. 
  TCCR1A =  0x81;         //OC1A - D10 compare match noninverting mode, downcounting ,PWM 8-bit 
}
//D10 PWM and D5 HIGH
void BH_AL(){  
  PORTD = B00000010;      //Set D5 to HIGH and the rest to LOW
  TCCR2A =  0;            // OC2A - D11 normal port. 
  TCCR1A =  0x81;         //OC1A - D10 compare match noninverting mode, downcounting ,PWM 8-bit 
}
//D11 PWM and D5 HIGH
...
//D11 PWM and D4 HIGH
void CH_BL(){  
  PORTD = B01000000;      //Set D5 to HIGH and the rest to LOW
  TCCR1A =  0;            // OC1A and OC1B normal port
  TCCR2A =  0x81;         // OC2A - D11 compare match noninverting mode, downcounting ,PWM 8-bit
}
...
ISR(PCINT2_vect){
  //Serial.println("PWM_INTERRUPT");
  //First we take the current count value in micro seconds using the micros() function
  current_count = micros();
  ///////////////////////////////////////Channel 1
  if(PIND & (1 << PIND2)){                              //We make an AND with the pin state register, We verify if pin 8 is HIGH???
